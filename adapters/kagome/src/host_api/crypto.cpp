...
namespace crypto {
  ...
  template <const char *CName, typename TPublicKey, typename TSignature>
  struct CryptoSuite {
  ...
  using Ed25519Suite =
      CryptoSuite<Ed25519Name, Ed25519PublicKey, Ed25519Signature>;
  using Sr25519Suite =
      CryptoSuite<Sr25519Name, Sr25519PublicKey, Sr25519Signature>;

  const kagome::runtime::WasmSize TEST_KEY_TYPE = 0xDEADBEEF;

  template <typename Suite>
  void public_keys(helpers::RuntimeEnvironment &environment,
  ...
  template <typename Suite>
  void generate(helpers::RuntimeEnvironment &environment,
                const std::vector<std::string> &inputs) {
    ...
    auto key = environment.execute<typename Suite::PublicKey>(
        "rtm_ext_crypto_"s + Suite::Name + "_generate_version_1",
        TEST_KEY_TYPE,
        std::make_optional(seed));

    std::cout << key.toHex() << std::endl;
  }

  template <typename Suite>
  void sign(helpers::RuntimeEnvironment &environment,
            const std::vector<std::string> &inputs) {
    BOOST_ASSERT(inputs.size() == 2);
    ...
    auto sig = environment
                   .execute<std::optional<typename Suite::Signature>>(
  ...
  template <typename Suite>
  void verify(helpers::RuntimeEnvironment &environment,
              const std::vector<std::string> &inputs) {
    BOOST_ASSERT(inputs.size() == 2);
