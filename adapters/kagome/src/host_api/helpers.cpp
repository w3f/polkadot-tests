...
public:
  WasmAdapterProvider() {
    ...
    std::ifstream file(WASM_ADAPTER_RUNTIME_PATH,
  ...
  const Buffer &
  getStateCodeAt(const kagome::primitives::BlockHash &at) const override {
    return code_;
  }
...
RuntimeEnvironment::RuntimeEnvironment() {
  ...
  auto trie_db =
      TrieStorageImpl::createEmpty(trie_factory, codec, serializer, boost::none)
          .value();
  ...
  auto crypto_store = std::make_shared<CryptoStoreImpl>(
      ...
      std::make_shared<Sr25519Suite>(sr25519_provider), bip39_provider,
  ...
  auto hostapi_factory = std::make_shared<HostApiFactoryImpl>(
      changes_tracker, sr25519_provider, ed25519_provider, secp256k1_provider,
  ...
  auto core_factory =
      std::make_shared<CoreFactoryImpl>(changes_tracker, header_repo);
  ...
  auto environment_factory = std::make_shared<RuntimeEnvironmentFactoryImpl>(
      core_factory, memory_factory, hostapi_factory, module_factory,
      wasm_provider, storage_provider, hasher);
